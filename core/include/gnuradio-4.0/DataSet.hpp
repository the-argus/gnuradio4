...
concept PacketLike = requires(T t) {
    ...
    requires std::is_same_v<decltype(t.signal_values), gr::Tensor<typename T::value_type>>;
    requires std::is_same_v<decltype(t.meta_information), gr::Tensor<typename T::pmt_map>>;
};
...
concept TensorLikeV2 = PacketLike<T> && requires(T t, const std::size_t n_items) {
    ...
    requires std::is_same_v<decltype(t.extents), gr::Tensor<std::int32_t>>;
...
concept DataSetLike = TensorLikeV2<T> && requires(T t, const std::size_t n_items) {
    ...
    // axis layout:
    requires std::is_same_v<decltype(t.axis_names), gr::Tensor<std::pmr::string>>;
    requires std::is_same_v<decltype(t.axis_units), gr::Tensor<std::pmr::string>>;
    requires std::is_same_v<decltype(t.axis_values), gr::Tensor<typename T::value_type, std::dynamic_extent, std::dynamic_extent>>;

    // signal data storage
    requires std::is_same_v<decltype(t.signal_names), gr::Tensor<std::pmr::string>>;
    requires std::is_same_v<decltype(t.signal_quantities), gr::Tensor<std::pmr::string>>;
    requires std::is_same_v<decltype(t.signal_units), gr::Tensor<std::pmr::string>>;
    requires std::is_same_v<decltype(t.signal_values), gr::Tensor<typename T::value_type>>;
    requires std::is_same_v<decltype(t.signal_ranges), gr::Tensor<Range<typename T::value_type>>>;

    // meta data
    requires std::is_same_v<decltype(t.meta_information), gr::Tensor<typename T::pmt_map>>;
    requires std::is_same_v<decltype(t.timing_events), gr::Tensor<std::pair<std::ptrdiff_t, gr::pmt::Value::Map>, std::dynamic_extent, std::dynamic_extent>>;
};
...
struct DataSet {
    ...
    // axis layout:
    gr::Tensor<std::pmr::string>                            axis_names{}; // axis quantity, e.g. time, frequency, …
    gr::Tensor<std::pmr::string>                            axis_units{}; // axis base SI-unit
    gr::Tensor<T, std::dynamic_extent, std::dynamic_extent> axis_values{};

    // signal data layout:
    gr::Tensor<std::int32_t> extents{}; // extents[dim0_size, dim1_size, …] i.e. [axis_values[0].size(), axis_values[1].size(), …]
    tensor_layout_type       layout{};  // row-major, column-major, “special”

    // signal data storage:
    gr::Tensor<std::pmr::string> signal_names{};      // defines number of signals, i.e. 'this->size()'
    gr::Tensor<std::pmr::string> signal_quantities{}; // size = this->size()
    gr::Tensor<std::pmr::string> signal_units{};      // size = this->size()
    gr::Tensor<T>                signal_values{};     // size = this->size() × Π_i extents[i]
    gr::Tensor<Range<T>>         signal_ranges{};     // [[min_0, max_0], [min_1, max_1], …] used for communicating, for example, HW limits

    // meta data
    gr::Tensor<pmt_map>                                               meta_information{};
    gr::Tensor<idx_pmt_map, std::dynamic_extent, std::dynamic_extent> timing_events{};
    ...
    [[nodiscard]] std::span<T>     axisValues(std::size_t axisIdx = 0UZ) {
        const std::size_t rowLength = axis_values.extent(1);
        return axis_values.data_span().subspan(_axCheck(axisIdx) * rowLength, rowLength);
    }
    [[nodiscard]] std::span<const T> axisValues(std::size_t axisIdx = 0UZ) const {
        const std::size_t rowLength = axis_values.extent(1);
        return axis_values.data_span().subspan(_axCheck(axisIdx) * rowLength, rowLength);
    }
    ...
    [[nodiscard]] std::span<idx_pmt_map> timingEvents(std::size_t signalIdx = 0UZ) {
        const std::size_t rowLength = timing_events.extent(1);
        return timing_events.data_span().subspan(_idxCheck(signalIdx) * rowLength, rowLength);
    }
    [[nodiscard]] std::span<const idx_pmt_map> timingEvents(std::size_t signalIdx = 0UZ) const {
        const std::size_t rowLength = timing_events.extent(1);
        return timing_events.data_span().subspan(_idxCheck(signalIdx) * rowLength, rowLength);
    }
...
struct Packet {
    ...
    std::int64_t        timestamp = 0;   // UTC timestamp [ns]
    gr::Tensor<T>       signal_values{}; // size = \PI_i extents[i
    gr::Tensor<pmt_map> meta_information{};
